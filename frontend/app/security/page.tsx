...
export default function SecurityPage() {
  ...
  async function load() { setSessions(await api<Session[]>("/auth/sessions")); setLoaded(true); }
  ...
  function changePassword(event: FormEvent) {
    ...
    action(async () => {
      await api("/auth/change-password", { method: "POST", body: JSON.stringify({ current_password: password, new_password: next }) });
      setPassword(""); setNext(""); setConfirm("");
  ...
  return <>
    ...
    <div className="security-grid">
      ...
      <section className="security-card"><h2>Confirmar identidade</h2><p>Confirme sua senha para autorizar ações administrativas sensíveis pelos próximos cinco minutos.</p>
        <form onSubmit={(event) => { event.preventDefault(); action(async () => {
          await api("/auth/reauthenticate", { method: "POST", body: JSON.stringify({ password: proof }) });
          setProof(""); setMessage("Identidade confirmada por cinco minutos.");
        }); }}>
