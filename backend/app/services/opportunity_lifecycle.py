...
TREATED = "treated"
REJECTED = "rejected"
REJECTION_REASONS = {"FALSE_POSITIVE", "OPERATIONAL_EXCEPTION", "ACCEPTABLE_COST", "RESOURCE_REQUIRED", "RISK_ACCEPTED", "OTHER"}
...
def _history(db: Session, finding: Finding, actor: User, *, to_status: str, action: str, reason: str | None = None, note: str | None = None) -> None:
    db.add(OpportunityStatusHistory(opportunity_id=finding.id, from_status=finding.status, to_status=to_status, action=action, reason=reason, note=note, changed_by=actor.id, changed_at=utcnow()))


def treat(db: Session, opportunity_id: str, actor: User, note: str | None = None) -> Finding:
...
def reject(db: Session, opportunity_id: str, actor: User, reason: str, note: str | None = None) -> Finding:
    ...
    _history(db, finding, actor, to_status=REJECTED, action="reject", reason=reason, note=note)
...
def reopen(db: Session, opportunity_id: str, actor: User, note: str | None = None) -> Finding:
    finding = _locked(db, opportunity_id)
    if finding.status not in {TREATED, REJECTED}:
        raise HTTPException(status_code=409, detail="Only treated or rejected opportunities can be reopened")
